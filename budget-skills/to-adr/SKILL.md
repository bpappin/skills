...
1. **Find where ADRs live.** Do not assume. Look for `docs/adr/` or
   ...
   If nothing exists yet, `docs/decisions/NNNN-short-slug.md` and say so
   rather than creating a convention silently.
...
`# Postgres over DynamoDB`, not `# ADR-0007: Postgres over DynamoDB`. An index of prefixed titles is
unreadable, and the number is already in the filename. The identifier belongs on the metadata line:
