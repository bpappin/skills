...
become a PRD (`to-prd`) and then stories (`to-stories`); once built and
stable, a Spec describes what exists.
...
Use the `research.md` template in this skill's `assets/templates/`. Four sections, and the second
...
2. **Number it, but keep the number out of the title.** The heading is the
   ...
   Enrichment`. An index full of prefixed titles is unreadable, and the
   number is already in the filename. The identifier goes on the
...
- Work that falls out → stories (`to-stories`), not a TODO list buried in the RAD.
