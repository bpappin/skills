...
1. **Project Wiring Rulebook (`WIRING.md`, repo root)** — pure agent
   machinery beside `AGENTS.md`, deliberately at the repo root rather than
   in the docs tree - it is read by agents, not by people. It defines:
...
1. **Code/docs scan** — scan the source tree, `docs/requirements/`, and
   the story files in `docs/stories/` to detect wiring candidates:
...
2. **Cross-reference** — compare the proposed feature's PRD and story
   against the integration triggers of all listed global services
   and features.
...
5. **Document in plan** — add an `Integration Hooks` section to the
   ...
   discovered work — write it as its own story (to-stories), don't fold it
