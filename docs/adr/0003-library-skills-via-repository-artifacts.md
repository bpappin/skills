...
Date: 2026-08-05 · Status: accepted (design); implementation not started
...
**Directory key is the skill's declared name** (`io.github.aughtone.types`),
...
MANAGED.md or a story-tools refresh will prune it as unknown - the same
...
mechanism while telling publishers not to use it is incoherent.** There is no
installed base to stay compatible with — nobody else publishes JVM library
skills today. And scanning was where all the cost and fragility lived: one
real cache held 7,462 jars and 555 AARs, each opened to check for a file that
...
```
src/agentSkills/<name>/SKILL.md          ← authored here
```
...
- A leading period is explicitly permitted ("it is acceptable to specify a
  ...
  fine. So `.agents/libraries/io.github.aughtone.types/` is safe.
...
- Skill sources move out of `resources/` in every library we publish. Any
  ...
  layout needs redoing against `src/agentSkills/`.
...
- `docs/outbox/jvm-agent-skills-proposal.md` - the outward-facing version
