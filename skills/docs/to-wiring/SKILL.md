---
name: to-wiring
description: Audit, check, and maintain the application's internal feature wiring rules in docs/spec/WIRING.md. Use when starting a project, proposing a new feature plan, or auditing feature-to-feature integration hooks.
license: MIT
metadata:
  author: bpappin
  version: "1.0"
...
1. **Project Wiring Rulebook (`docs/spec/WIRING.md`)** — a spec document
...
2. **AI instructions (`AGENTS.md`)** — rules in the project's root agent
   ...
   `docs/spec/WIRING.md` when planning new code.
...
command runs and `docs/spec/WIRING.md` does not exist.
...
2. **Generate WIRING.md** — load this skill's
   ...
   user's answers, and write the result to `docs/spec/WIRING.md`
   (create `docs/spec/` if needed).
...
1. **Code/docs scan** — scan the source tree, `docs/prd/`, and the
...
5. **Inject AGENTS.md rules** — ensure the project's root `AGENTS.md`
   ...
   - **Wiring & Integration Checks**:
     ...
     - Suggest adding the new feature to `docs/spec/WIRING.md` if it
...
1. **Read rules** — read `docs/spec/WIRING.md`.
