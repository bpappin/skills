...
| Group work by topic | The project's own groupings live in the **Topical Tags** field, values listed in `.agents/config/dimensions.md`. Reuse one before adding. To add: append a line to `.agents/config/topical-tags.md` (the agents' list - it is committed and reviewed), then `story-reconcile/scripts/yt-pull.sh --dimensions-only --push-tags` carries it into the tracker. Nothing constrains what a tag looks like; name it so it groups the work usefully. Server tags are for cross-cutting state and the one-off batch tag (above); a grouping that will still mean something in six months belongs here. |
