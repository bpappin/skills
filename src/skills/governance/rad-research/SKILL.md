---
name: rad-research
description: Manage Technical Research & Development (R&D) logs. Use this to start new research entries, document hypotheses, log architectural decisions, and maintain the docs/research/ index.
---
...
## Activation Guard
Before executing any workflow, the agent MUST verify that `"rad_enabled": true` is set in the `.config/project.json` file in the workspace root. If it is false or missing, the agent must inform the user that R&D logging is disabled for this project and only proceed if explicitly instructed.
